...
//                    + p.getProfessionNo());
    ...
    @Override
    public boolean changePassBySQL(Admin ad, String sql) {
        // update password
        session = getSession();

        // 根据sql更新密码
        Query exe = session.createSQLQuery(sql);
        exe.setParameter(0, ad.getPassword());
        exe.setParameter(1, ad.getAccount());
        int num = exe.executeUpdate();
        session.close();
        if (num == 1) {
            return true;
