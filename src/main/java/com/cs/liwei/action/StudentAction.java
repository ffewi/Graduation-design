...
public class StudentAction extends ActionSupport {
	...
	public String ajaxTermChange() {
		...
		ssgList = student.getStuMsgByTermAndByPageNo(ssgForm.getStudentNo(),
				ssgForm.getTerm(), 1);
	...
	public String ajaxSearchByCourName() {
		...
		ssgList = student.searchStuMsgByTermAndByCourseName(
				ssgForm.getStudentNo(), ssgForm.getTerm(),
				ssgForm.getCourseName());
		JSONArray json = JSONArray.fromObject(ssgList);
		try {
			ServletActionContext.getResponse().setCharacterEncoding("UTF-8");
			ServletActionContext.getResponse().getWriter()
	...
	public String ajaxNextPage() {
		System.out.println(ssgForm.getStudentNo() + ":  " + ssgForm.getTerm()
				+ ": " + pageMsg.getPageNo());
		ssgList = student.getStuMsgByTermAndByPageNo(ssgForm.getStudentNo(),
				ssgForm.getTerm(), pageMsg.getPageNo());
		JSONArray json = JSONArray.fromObject(ssgList);
		try {
			ServletActionContext.getResponse().setCharacterEncoding("UTF-8");
			ServletActionContext.getResponse().getWriter()
	...
	public String indexPage() {
		...
		if (termList != null) {
			...
			ssgList = student.getStuMsgByTermAndByPageNo(stu.getStudentNo(),
					firstIndex, 1);
			if (ssgList != null) {
