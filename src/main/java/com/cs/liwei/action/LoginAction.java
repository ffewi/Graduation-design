...
public class LoginAction extends ActionSupport {
    ...
    /*
     * public String invokeUser() { ActionContext.getContext().getSession().put("用户名：可以定义一个常量", "getUsername()");
     * ActionContext.getContext().getSession().put("用户密码：可以定义一个常量", "getPassword()");
     * ActionContext.getContext().getSession().put("用户：可以定义一个常量", "user:我也可以存一个对象"); return "设置 登陆用户"; // 记得在struts.xml 中配置 拦截器
     * 
     * }
     */
    ...
    public String execute() {
        ...
        case UserForm.STU_TYPE:
            ...
            if (result != null) {
                isLogin = (result.getStudentNo() == stu.getStudentNo() && result.getStuPass()
                        .equals(stu.getStuPass()));
            }
            if (isLogin) {
