...
public class AdminManagerImpl implements AdminManager {
	...
	public List<ProForm> addPro(ProForm proForm) {
		// 逻辑代码，传入的值只有
		// 院系编号proForm.getSelectArr()[0]，和专业名称proForm.getProfessionName;
	...
	public int getPageTotal(int categoryOfMethod) {
		...
		case Method.PAGE_DETP:
			int totalDept = dao.countDept();
			pages = (totalDept % 10 == 0) ? (totalDept / 10)
					: (totalDept / 10) + 1;
			break;
		case Method.PAGE_PROFESSION:
			int totalPro = dao.countPro();
			pages = (totalPro % 10 == 0) ? (totalPro / 10)
					: (totalPro / 10) + 1;
			break;
		case Method.PAGE_COURSE:
			int totalCourse = dao.countCourse();
			pages = (totalCourse % 10 == 0) ? (totalCourse / 10)
					: (totalCourse / 10) + 1;
			break;
