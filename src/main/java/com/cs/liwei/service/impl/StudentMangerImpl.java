...
public class StudentMangerImpl implements StudentManager {
	...
	public List<ScoreForm> addScoreByScore(ScoreForm sf) {
		...
		s.setExamType("闭卷");
		int fenshu = CountTotalGrade.finalScore(sf.getPingshiScore(),
				sf.getExamScore());
	...
	@Override
	public List<StuShowGrade> getStuMsgByTermAndByPageNo(int studentNo,
			int term, int pageNo) {
		// 调用dao层 获取 学生 成绩信息 区分 term
		List<StuShowGrade> list = dao.getStuShowGradeByTerm(studentNo, term,
				pageNo);
		if (null != list && !list.isEmpty()) {
			return list;
		}
	...
	@Override
	public List<StuShowGrade> searchStuMsgByTermAndByCourseName(int studentNo,
			int term, String courseName) {
		// 调用dao层 模糊查询学生 以学期为单位的课程
		List<StuShowGrade> list = dao.getStuSearchByTermAndCourseName(
				studentNo, term, courseName);
		if (null != list && !list.isEmpty()) {
			return list;
		}
		return null;
	...
	@Override
	public boolean changePassword(int studentNo, String pass) {
		// 调用dao层修改密码
		boolean isOk = dao.changePassword(studentNo, pass);
